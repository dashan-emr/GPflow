...
class LMC_TSK(GPflow.model.Model):
    def __init__(self, num_tasks,rank,num_latent_list,kern_list, likelihood_list,
                  ...
                  W = None, Kappa = None,locMin = 0, locMax = 5000,X = None, Y = None, lagging=None,
                  w_log = None, length_log = None, lamda = 1.):
        """
        ***** LMC + Task-Specific Kernel (TSK) *****
        ...
        self.tsk = TSK
        self.w_log,self.length_log = w_log,length_log
    ...
    """    
    def _unfix_all_params(self):

        #self.reset_params()
        #unfix global (except variances and Z)

        self.W.fixed = False
        if self.tsk:
            self.Kappa.fixed = False
        for d in range(self.num_tasks):
            self.likelihood[d].reset_params()
        for kern in self.kern_list:
            kern.variance.fixed = True
            kern.lengthscales.fixed = False
        self.lagging.fixed = False

        #unfix local (except variances and Z)
        for q_mu in self.q_mu_list: q_mu.fixed = False
        for q_sqrt in self.q_sqrt_list: q_sqrt.fixed = False
    ...
    def _compile(self):
        ...
        if self.tsk:
            for kern in self.tskern_list:
                if not kern.lengthscales.fixed:
                    ...
                    #self.tsk_indices.extend(kern.lengthscales.free_array_ind) """ Question: what is tsk_indices """
        ...
        """
            TODO:
            Keep track of global indexes, let's test on w and kernel lengthscales first.
            The result of the lengthscales_ind is a flat list
        """
        #self.lengthscales_ind = []
        for kern in self.kern_list:
            if not kern.lengthscales.fixed:
                self.global_indices.extend(kern.lengthscales.free_array_ind)
        ...
        """
            NOTE: free_array_ind is NOT an integer but a list of integers
            self.w_ind is an list of integers.
        """
        #self.w_ind = self.W.free_array_ind
    ...
    def _sgd(self, X_list, Y_list, max_iters, local_iters, method, options):
        ...
        while _continue and step < max_iters:
            ...
            try:
                ...
                self.set_state(var)
                #self.w_log.append(var[self.W.free_array_ind])
                self.length_log.append(var[np.array([kern.lengthscales.free_array_ind for kern in self.kern_list]).flatten()])
