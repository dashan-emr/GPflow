...
class LMC_TSK(GPflow.model.Model):
    def __init__(self, num_tasks,rank,num_latent_list,kern_list, likelihood_list,
                  ...
                  whiten_list = None,needs_recompile = True, num_inducing_points = 10,
                  invariant = None, event = None, T = None, h_fun = None,
                  W = None, Kappa = None,locMin = 0, locMax = 5000,X = None, Y = None, lagging=None,lamda = 1,file_name = 'param.txt'):
        """
        ***** LMC + Task-Specific Kernel (TSK) *****
        ...
        GPflow.model.Model.__init__(self,name = 'LMC_TSK_JOINT')
        ...
        self.lamda = lamda
        print "lamda is", self.lamda
        self.file_name = file_name

        """ Time to Event Data """
        self.invariant = invariant; self.event = event
        self.gamma = Param(np.random.randn(len(self.num_tasks),1))
        self.alpha = Param(np.random.randn(len(self.num_tasks),1))
        self.T = T  # T is the time to event last observation time, T is a list of list [[[100]],[[110]],[[120]],[[123]]]
        if h_fun is None:
            h_fun = lambda x:x 
    ...
    def _compile(self):
        ...
        if self.tsk:
            for kern in self.tskern_list:
                if not kern.lengthscales.fixed:
                    ...
                    #self.local_indices.extend(kern.variance.free_array_ind)  uncomment that
        ...
        for kern in self.kern_list:
            if not kern.lengthscales.fixed:
                ...
                #self.global_indices.extend(kern.variance.free_array_ind) uncomment that
    ...
    def _sgd(self, X_list, Y_list, T_list, event_list, invariant_list, max_iters, local_iters, method, options):
        """
        ...
        while _continue and step < max_iters:
            ...
            try:
                ...
                feed_event = {self.event_t: event_list[order]}
                feed_invariant = {self.invariant_t: invariant_list[order]}
                feed_T = {self.T: T_list[order]}
                feed_dict.update(feed_1);feed_dict.update(feed_2);feed_dict.update(feed_event);feed_dict.update(feed_invariant);feed_dict.update(feed_T)
                t0 = time.time()
                ...
                fp = open(str1,'ab')
                np.savetxt(fp, var[self.global_indices].reshape(-1,1).T, '%f')
                fp.close()
    ...
    def optimize(self, max_iters, local_iters, X_list, Y_list, T_list, event_list, invariant_list,method, options):

        local_vars, global_vars = self._sgd(X_list, Y_list, T_list, event_list, invariant_list, max_iters, local_iters, method = method, options = options)
